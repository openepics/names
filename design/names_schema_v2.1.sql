...
CREATE  TABLE IF NOT EXISTS `name` (
  ...
  `is_retired` TINYINT(1) NOT NULL DEFAULT false COMMENT 'name has been deleted' ,
...
CREATE  TABLE IF NOT EXISTS `change_request` (
  ...
  `new_code` VARCHAR(16) NOT NULL COMMENT 'new code' ,
  `new_category_id` VARCHAR(16) NOT NULL COMMENT 'new category' ,
  `new_description` VARCHAR(255) NOT NULL COMMENT 'new description' ,
  `version` INT NOT NULL DEFAULT 0 COMMENT 'For optimistic concurrency control.' ,
  INDEX `fk_change_request_name1` (`name_id` ASC) ,
  INDEX `fk_change_request_category1` (`new_category_id` ASC) ,
  PRIMARY KEY (`id`) ,
  CONSTRAINT `fk_change_request_name1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_change_request_category1`
...
START TRANSACTION;
USE `discs_names2`;
INSERT INTO `configuration` (`name`, `value`) VALUES ('version', '2.1');
