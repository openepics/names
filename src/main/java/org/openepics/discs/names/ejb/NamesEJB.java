 ...
 * 
 */
package org.openepics.discs.names.ejb;
...
import org.openepics.discs.names.ent.Privilege;
import org.openepics.discs.names.ui.UserManager;
...
public class NamesEJB {
    ...
    private static final Logger logger = Logger.getLogger(NamesEJB.class.getName());
     ...
     * 
     * @param category         Restrict names to the given name-element category
     * @param includeAll       Include names with any status, not just approved ones
     * 
     * 
     */
    
    // @TransactionAttribute(TransactionAttributeType.SUPPORTS) // No transaction as it is read-only query
    public List<NameEvent> findNames(String category, boolean includeAll) {
        List<NameEvent> nameEvents;
        TypedQuery<NameEvent> query;

        //TypedQuery<NameEvent> query = em.createQuery("SELECT n FROM NameEvent n WHERE n.status = 'a' AND n.eventType IN (?1, ?2) AND n.nameCategoryId.id LIKE ?3 AND n.processDate <= (SELECT MAX(r.releaseDate) FROM NameRelease r) ORDER BY n.nameCategoryId.id, n.nameCode", NameEvent.class)
        if (includeAll) {
            query = em.createQuery("SELECT n FROM NameEvent n WHERE n.nameCategoryId.id LIKE :categ AND n.requestDate = (SELECT MAX(r.requestDate) FROM NameEvent r WHERE r.nameId = n.nameId ) ORDER BY n.nameCategoryId.id, n.nameCode", NameEvent.class)
                    .setParameter("categ", category); // ToDo: convert to criteria query.
        } else {
            query = em.createQuery("SELECT n FROM NameEvent n WHERE n.nameCategoryId.id LIKE :categ AND n.requestDate = (SELECT MAX(r.requestDate) FROM NameEvent r WHERE r.nameId = n.nameId AND r.status = 'a') AND NOT (n.eventType = 'd' AND n.status = 'a') ORDER BY n.nameCategoryId.id, n.nameCode", NameEvent.class)
                    .setParameter("categ", category); // ToDo: convert to criteria query.
        }
        // ToDo: check category values
        nameEvents = query.getResultList();
